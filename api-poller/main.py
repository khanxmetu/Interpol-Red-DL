...
from api_poller.query_strategy import QueryStrategyFactory
...
class APIPoller:
    ...
    def _read_and_publish_notice(self, notice_id: str) -> bool:
        ...
        try:
            notice: Notice = self._notice_detail_fetcher.fetch_notice_detail(notice_id)
        ...
        try:
            ...
            print(f"[-] Failed to publish notice: {notice_id}. Reason: {e}")
            return False
    ...
    def _read_and_publish_notices(self) -> int:
        ...
        for query_options in self._query_options_list:
            try:
                notice_ids = self._notice_list_fetcher.fetch_notice_ids(query_options)
            except APIPollerException as e:
                print(
            ...
            print(f"[+] {len(notice_ids)} notices found with query: {query_options}")
            if len(notice_ids) == 160:
                print(
                    f"[!] Number of notices may have exceed for query: {query_options}"
                )

            for notice_id in notice_ids:
                count += self._read_and_publish_notice(notice_id)
                print(f"[+] Waiting {self._api_rate_limit_delay}s for next read...")
    ...
    def run(self):
        ...
        while True:
            ...
            print(
                f"[+] {count} notices published successfully in {end_time-start_time} seconds"
            )
            print(f"[+] Waiting {self._poll_interval}s for next poll run...")
...
def main():
    ...
    session_headers = json.loads(open("config-data/headers.json").read())
    rabbitmq_host = os.environ["RMQ_HOST"]
    rabbitmq_port = int(os.environ["RMQ_PORT"])
    rabbitmq_user = os.environ["RMQ_USER"]
    rabbitmq_pass = os.environ["RMQ_PASS"]
    queue_name = os.environ["RMQ_NOTICE_QUEUE"]

    query_strategy_id = int(os.environ["QUERY_STRATEGY"])
    query_strategy = QueryStrategyFactory.get_by_id(query_strategy_id)
    ...
    notice_list_fetcher = NoticeListFetcher(session, red_list_url)
    notice_img_ids_fetcher = NoticeImageIDsFetcher(session, red_list_url)
    notice_detail_fetcher = NoticeDetailFetcher(
        session, red_list_url, notice_img_ids_fetcher
    )
