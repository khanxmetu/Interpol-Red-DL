...
class ArrestWarrant(EmbeddedDocument):
    charge = StringField(max_length=500)
    issuing_country_id = StringField(choices=COUNTRIES)
    charge_translation = StringField(max_length=500)
...
class Notice(Document):
    ...
    weight = IntField(min_value=10, max_value=150)
    ...
    place_of_birth = StringField(max_length=50)
    forename = StringField(max_length=50)
    ...
    languages_spoken_ids = ListField(StringField(choices=SEXES))
    height = FloatField(min_value=1, max_value=2)
    img_ids = ListField(StringField(max_length=20))
